...
function BudgetCard({ budget }) {
  ...
  const { formattedTotal, breakdown = [] } = budget;
        ...
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <div className="flex items-center gap-1 text-2xl font-bold text-emerald-400">
              ...
              <span>{formattedTotal.replace('$', '')}</span>
            </div>
          </div>
          ...
          {breakdown.map((item, idx) => (
            <div key={idx} className="space-y-1">
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-400 font-mono">${item.amount.toLocaleString()} ({item.percentage}%)</span>
              </div>
